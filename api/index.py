...
# DO NOT label braids as "wavy" or "wig".
# If the hair is divided into repeated woven/plaited strands, classify it as "braids".
# If medium/large individual braids are visible, classify as "box braids" or "knotless braids".
# If unsure between wig and braids, choose "braids".
# Hair color, highlights, or curls at the ends do NOT change the protective style label.
...
#     # This is to keep the profile filled while still improving consistency across 3 photos.
#     if should_update:
...
UNCERTAIN_VALUES = {"uncertain", "not clearly visible", ""}
...
face shape: {profile.get('face_shape')}
hair: {profile.get('hair_texture')}
"""


def to_data_url(file_bytes, mime):
...
# HAIR FIX (CRITICAL)
# -----------------------------
def fix_hair_label(result):
    hair = result.get("hair_texture", "").lower()
    notes = " ".join(result.get("notes", [])).lower()

    if "braid" in notes:
        return "braids"

    if hair in ["wig", "wavy", "curly"]:
        return "braids"
...
# ANALYZE FACE
# -----------------------------
def analyze_face_photo(image_bytes, mime, profile):

    data_url = to_data_url(image_bytes, mime)

    prompt = f"""
You are analyzing a face photo.

CRITICAL RULES:
- If you see ANY braided pattern → return "braids"
- NEVER call braids "wavy" or "wig"
- If unsure between wig and braids → choose "braids"
...
# APPLY TO PROFILE
# -----------------------------
def apply_to_profile(analysis, profile):
    updated = profile.copy()

    for key in ["face_shape", "skin_tone", "undertone", "hair_texture"]:
        val = analysis.get(key, "").lower()
        if val and val not in UNCERTAIN_VALUES:
            updated[key] = val
...
async def analyze_face(
    ...
    result = analyze_face_photo(image_bytes, file.content_type, current_profile)

    updated_profile = apply_to_profile(result, current_profile)
